...
def helpmenu():
    while True:
        ...
        font = pygame.font.SysFont('Corbel', 30)
        text2 = font.render('W,A,S,D : UP,DOWN,LEFT,RIGHT', 1, (115,
                            151, 193))
        window.blit(text2, (100, 125))
        text3 = font.render('HOLD LEFT CLICK : BOAT FOLLOWS CURSOR', 1,
                            (115, 151, 193))
        window.blit(text3, (35, 175))
        text4 = font.render('BACKSPACE : RETURN TO MAIN MENU', 1, (115,
                            151, 193))
        window.blit(text4, (60, 225))
        text4 = font.render('+,- : INCREASE/DECREASE GOBLIN SPEED', 1,
                            (115, 151, 193))
        window.blit(text4, (50, 275))
        font = pygame.font.SysFont('Corbel', 15)
        text5 = \
            font.render('THE OBJECTIVE IS TO GET THE BOAT (THE BLACK CIRCLE) TO SHORE '
                        , 1, (115, 151, 193))
        window.blit(text5, (90, 70))
        text6 = \
            font.render('(THE LIGHT BLUE REGION) WITHOUT BEING CAUGHT BY THE GOBLIN (THE RED DOT).'
                        , 1, (115, 151, 193))
...
def clearSquare():
    ...
    pygame.draw.rect(window, (115, 151, 193), pygame.Rect(width / 2
                     - 100, height / 2 - 100, 200, 200))
...
def redrawSquare(draw_text=False, win=False):
    ...
    pygame.draw.circle(window, (255, 0, 0), (int(width / 2 + boatx),
                       int(height / 2 + boaty)), 6, 2)

    # goblin Square

    pygame.draw.circle(window, (0, 0, 0), (int(width / 2 + goblinx),
                       int(height / 2 + gobliny)), 6, 0)
...
def clearTri():
    ...
    pygame.draw.polygon(surface=window, color=(115, 151, 193),
                        points=[(112, 250), (262, 75), (412, 250)])
...
def redrawTri(draw_text=False, win=False):
    ...
    pygame.draw.circle(window, (255, 0, 0), (int(width / 2 + goblinx),
                       int(height / 2 + gobliny)), 6, 0)
...
def detectWinTri():
    ...
    if boaty >= 71 or boaty <= 1.1 * boatx - 100 and boatx >= 0 \
        or boaty <= -1.1 * boatx - 100 and boatx < 0:
        while True:
            is_win = abs(diff) > 10
